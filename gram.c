...
int editorRowRxToCx(erow *row, int rx)
{
	...
	{
		if (row->chars[cx] == '\t')
			cur_rx += (GRAM_TAB_STOP - 1) - (cur_rx % GRAM_TAB_STOP);
		cur_rx++;
		...
		if (filerow >= E.numrows)
		{
			if (E.numrows == 0 && y == E.screenrows / 3)
			{
				...
				int welcomelen = snprintf(welcome, sizeof(welcome),
										  "Kilo editor -- version %s", GRAM_VERSION);
...
{
	E.cx = 0;
	// the number of charachters in the rows
	E.cy = 0;
	E.rx = 0;
	// will also include the number of tabbings i.e. spaces in the rows
	E.rowoff = 0;
	E.coloff = 0;
	E.numrows = 0;
	E.row = NULL;
